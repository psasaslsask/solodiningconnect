...
export default function RestaurantList() {
  ...
  const [dealsByRestaurant, setDealsByRestaurant] = useState({});
  ...
  // Live deals stream from restaurant owners
  useEffect(() => {
    if (!restaurants.length) return undefined;

    const unsubscribes = restaurants.map((restaurant) => {
      ...
      return onSnapshot(dealsRef, (snapshot) => {
        ...
        setDealsByRestaurant((prev) => ({ ...prev, [restaurant.id]: deals }));
      });
    });
          ...
          {restaurants.map((r) => (
            ...
            >
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                {dealsByRestaurant[r.id]?.length > 0 && (
                  <div className="mt-4 bg-amber-50 border border-amber-100 rounded-2xl p-3 space-y-2">
                    <div className="text-xs font-semibold text-amber-800 uppercase tracking-[0.14em] flex items-center gap-2">
                      Solo diner perks
                      <span className="px-2 py-0.5 rounded-full bg-white border border-amber-200 text-[11px] text-amber-700">
                        {dealsByRestaurant[r.id].length} live
                      </span>
                    </div>
                    {dealsByRestaurant[r.id].slice(0, 2).map((deal) => (
        ...
        <div className="fixed inset-0 bg-slate-900/60 flex justify-center items-center z-50 px-4">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-lg border border-slate-200 p-6">
            ...
            <div className="space-y-3">
              {dealsByRestaurant[selectedRestaurant.id]?.length > 0 && (
                <div className="bg-amber-50 border border-amber-100 rounded-2xl p-3 space-y-1">
                  <p className="text-xs font-semibold text-amber-800 uppercase tracking-[0.14em]">Current deals</p>
                  {dealsByRestaurant[selectedRestaurant.id].slice(0, 2).map((deal) => (
